...
## 1.2 Глоссарий
1. База длинной арифметики - это максимальное число хранимое в массиве длинной арифметики

2. **Определение.** Комплексные числа — это числа вида $a + bi$, где $a$ и $b$ это обычные вещественные числа, а $i$ это так называемая мнимая единица: это число, для которого выполняется равенство $i^2 = -1$. 
...
### В данной работе требуется 
 ...
 - Изучиить политературным источником и описать его в форме научного доклада
 - Реализовать алгоритм с использованием языков программирования С++ и Python
 - В моей длинной арифметике будет ограничение в 300000 символов
...
```
Сначала складываем отдельные разряды, начиная с конца, строка 9; делим получившуюся сумму на 10 и получаем целую часть от деления на десять, которую мы сразу прибавляем к следующему разряду. Далее мы отсекаем первый разряд полученного числа, если, он есть. В самом конце нужно определить знак.

## 4.2 Вычитание длинных чисел
---
Для начала нужно определить какое число больше, если числа одинаковой длинны, то переходим к по разрядовому сравнению
...
``` 
Создаём массив размером 2*len(где len это длинна первого числа)
Умножаем число как в школьной арифметике
...
Мы его брать не будем, так как он недостачно быстрый Асимптотика O($n^2$)
...
Для генерирования длинных чисел я буду использовать функцию random(seed) на питоне
Для проверки правильности ответов я буду исполтзовать длинную арифметику в питоне
